...
if (isset($_SESSION["lobby"]) && isset($_SESSION["player"])) {
	...
	$datas = "[";
	foreach($dbh->query("SELECT * FROM game_datas WHERE player_id IN (SELECT id FROM players WHERE lobby_id = $lobby_id AND is_playing = 1) AND player_id != $player_id") as $row) {
